...
export async function loadCocosStudioFromLocalFiles(fileList) {
    ...
    // Find json
    const jsonFile = files.find(f => f.name.endsWith('.ExportJson') || f.name.endsWith('.json'));
    
    // Find plists and pngs
    const plistFiles = files.filter(f => f.name.endsWith('.plist'));
    ...
    for (const plistFile of plistFiles) {
        ...
        if (plistData.frames) {
            ...
            const pngFile = files.find(f => f.name === textureFileName);
            if (!pngFile) {
